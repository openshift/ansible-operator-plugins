...
assumptions (beyond the RPM-exclusion list).
...
Stage 6 — Verify that every package pinned in Pipfile.lock is accounted for in
           requirements.txt (active, unless it's RPM-installed, in which case
           it must be commented out; pip/setuptools/distribute are expected to
           be entirely absent, matching pip-compile's own unsafe-package
           handling).  Exits non-zero on any discrepancy.
...
# pip-tools' own reserved "unsafe" package names (see piptools.utils.UNSAFE_PACKAGES).
# Stage 2 compiles requirements.txt WITHOUT --allow-unsafe, so pip-compile silently
# drops any of these from its output even if they appear in `pip freeze --all`
# (which, unlike plain `pip freeze`, always includes pip/setuptools/distribute).
# They are expected to be entirely absent from requirements.txt — this is
# correct, pre-existing behaviour, not something Stage 6 should flag.
PIP_COMPILE_UNSAFE: frozenset[str] = frozenset({
    "pip",
    "setuptools",
    "distribute",
})
...
def auto_fix_cves(rpm_installed: frozenset[str]) -> None:
    ...
    Three outcomes per CVE:
      ...
      pip package, Pipfile   — the version constraint in Pipfile is too narrow
        constraint prevents    to reach the safe version; remediation guidance
        the fix                is printed and the script exits non-zero.
    """
    ...
    check_output = _strip_ansi(check.stdout + "\n" + check.stderr)
    vulns = _parse_vuln_report(check_output)
    if not vulns:
    ...
    recheck_output = _strip_ansi(recheck.stdout + "\n" + recheck.stderr)
    remaining_pip = [
        v for v in _parse_vuln_report(recheck_output)
        if _norm(v["package"]) not in rpm_norms
    ]
    if remaining_pip:
        print(
            f"\n  FATAL: {len(remaining_pip)} CVE(s) remain in pip-managed packages.\n"
        ...
        print(
            ...
            " best-effort fix.)\n"
            "  Failing the build so this isn't silently missed — if a CVE genuinely"
            " cannot be fixed right now, update RPM_INSTALLED or the affected"
            " Pipfile constraint deliberately, or accept the risk explicitly by"
            " adjusting this check rather than leaving it as a buried warning.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def stage1_resolve_runtime(tmp: Path) -> str:
    ...
    # Exits non-zero if a pip-managed CVE remains after the auto-fix attempt.
...
def stage5_check_build_dep_cves(out_dir: Path, tmp: Path) -> None:
    ...
    rpm_norms = {_norm(p) for p in RPM_INSTALLED}
    unresolved: list[tuple[str, str, str, str]] = []  # (label, pkg, vuln_id, reason)
    ...
    for phase_key, txt_path, in_path in build_phases:
        ...
        for safety_cmd in (
            ...
            try:
                ...
                continue   # executable itself missing — try next form
            stderr_lower = check.stderr.lower()
            if "no such file" in stderr_lower or "not found" in stderr_lower:
                # e.g. "pipenv run" reports the wrapped command couldn't be found
                continue   # try next form
            break   # got a real result (success or a genuine vuln/error report)

        if check is None:
            print(
                f"  WARNING: could not locate a 'safety' executable to scan"
                f" {txt_path.name}; skipping.",
                file=sys.stderr,
            )
            continue
        ...
        for vuln in pip_vulns:
            ...
            if not min_safe:
                ...
                unresolved.append(
                    (label, pkg, vid, f"could not infer min safe version from '{spec}'")
                )
                continue
            ...
            if ok:
                ...
                unresolved.append(
                    (label, pkg, vid, f"blocked by conflicting constraint on '{conflict}'")
                )
    ...
    if unresolved:
        print(
            f"\n  FATAL: {len(unresolved)} CVE(s) in build requirements could not be"
            " auto-fixed:",
            file=sys.stderr,
        )
        for label, pkg, vid, reason in unresolved:
            print(f"    • [{label}] {pkg} [{vid}]: {reason}", file=sys.stderr)
        print(
            "  Failing the build so this isn't silently missed — resolve the"
            " underlying constraint conflict (or update the Pipfile/build-dep"
            " metadata) and re-run 'make generate-requirements'.",
            file=sys.stderr,
        )
        sys.exit(1)
...
def stage6_verify_completeness(all_pinned: dict[str, str], out_dir: Path) -> None:
    """
    Sanity-check that every package pinned in Pipfile.lock (i.e. every entry
    Stage 1 resolved via `pip freeze --all`) is accounted for in
    requirements.txt: present and *uncommented* unless it's in RPM_INSTALLED
    (must be present but *commented out*) or PIP_COMPILE_UNSAFE (pip-compile
    itself always drops these when run without --allow-unsafe, as Stage 2
    does for requirements.txt, so they are expected to be entirely absent).

    This catches regressions where a package is silently missing, omitted, or
    left commented out (or wrongly left active) by a bug in Stage 2's
    conflict-exclusion, manual-append, or RPM-comment-out logic.

    Exits non-zero with a detailed report if any package fails the check.
    """
    print("\n══ Stage 6: Verify requirements.txt completeness ══")

    req_txt = out_dir / "requirements.txt"
    if not req_txt.exists():
        print(
            f"  FATAL: {req_txt} does not exist; cannot verify completeness.",
            file=sys.stderr,
        )
        sys.exit(1)

    rpm_norms = {_norm(p) for p in RPM_INSTALLED}
    unsafe_norms = {_norm(p) for p in PIP_COMPILE_UNSAFE}

    present: dict[str, bool] = {}  # norm_name -> True if active (uncommented)
    for raw in req_txt.read_text().splitlines():
        line = raw.strip()
        commented = line.startswith("#")
        candidate = line.lstrip("#").strip()
        m = re.match(r"^([A-Za-z0-9][A-Za-z0-9._-]*)==", candidate)
        if not m:
            continue
        norm = _norm(m.group(1))
        # If a package somehow appears both commented and uncommented, treat
        # it as active — pip will install it either way.
        present[norm] = present.get(norm, False) or not commented

    missing: list[str] = []
    wrongly_commented: list[str] = []
    wrongly_active: list[str] = []
    unexpectedly_present: list[str] = []

    for norm in sorted(all_pinned):
        if norm in unsafe_norms:
            # pip-compile drops these unconditionally without --allow-unsafe;
            # their presence would indicate an unexpected pip-tools behaviour
            # change, not a Stage 2 bug, so just note it rather than failing.
            if norm in present:
                unexpectedly_present.append(norm)
            continue
        if norm not in present:
            missing.append(norm)
            continue
        active = present[norm]
        should_be_active = norm not in rpm_norms
        if should_be_active and not active:
            wrongly_commented.append(norm)
        elif not should_be_active and active:
            wrongly_active.append(norm)

    if not (missing or wrongly_commented or wrongly_active):
        print(
            f"  ✓ All {len(all_pinned) - len(unsafe_norms & set(all_pinned))}"
            " applicable Pipfile.lock packages accounted for in requirements.txt"
            f" ({len(unsafe_norms & set(all_pinned))} pip-compile-unsafe package(s)"
            " correctly excluded)"
        )
        if unexpectedly_present:
            print(
                f"  NOTE: {sorted(unexpectedly_present)} appeared in requirements.txt"
                " despite being in PIP_COMPILE_UNSAFE — harmless, but worth a look.",
                file=sys.stderr,
            )
        return

    print(
        f"\n  FATAL: requirements.txt completeness check failed"
        f" ({len(all_pinned)} packages expected):",
        file=sys.stderr,
    )
    if missing:
        print(f"    Missing entirely ({len(missing)}): {sorted(missing)}", file=sys.stderr)
    if wrongly_commented:
        print(
            f"    Present but commented out, should be active ({len(wrongly_commented)}):"
            f" {sorted(wrongly_commented)}",
            file=sys.stderr,
        )
    if wrongly_active:
        print(
            f"    Present but active, should be commented out as RPM-installed"
            f" ({len(wrongly_active)}): {sorted(wrongly_active)}",
            file=sys.stderr,
        )
    print(
        "  Every package pinned in Pipfile.lock must appear in requirements.txt,"
        " active unless it is in RPM_INSTALLED (in which case it must be"
        " commented out) or in PIP_COMPILE_UNSAFE (in which case it must be"
        " absent). Investigate Stage 2's exclusion/append/comment-out logic for"
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="gen_reqs_") as td:
        ...
        # Stage 6 — verify no Pipfile.lock package is missing/omitted from
        # requirements.txt (sanity check requested in review)
        stage6_verify_completeness(all_pinned, out_dir)
