...
Stage 4  │  Conflict detection + phase splitting  →  3 build .txt files
         ...
         │
Stage 5  │  Safety CVE scan of build files  →  auto-fix or warn
         │
...
| Pipfile constraint prevents fix | Warn with specific guidance; re-run after updating the Pipfile |
...
"Broaden the Pipfile constraint…" and require a manual Pipfile edit.
...
At each recursion level, five strategies are tried in order:
...
phase becomes build1 (or remains separate). This prevents blindly merging
phases that have their own internal conflicts.
...
Stage 4 solves this automatically with a three-step process:
